...
//TOP SENSORS
#define sens1 2  //IR 1 - Top Left conrner
#define sens2 1  //IR 2 - Top Center
#define sens3 0  //IR 3 - Top Right Corner

//BOTTOM SENSORS
#define sens4 5  //IR 4 - Bottom Left Corner
#define sens5 4  //IR 5 - Bottom Center
#define sens6 3  //IR 6 - Bottom Right Corner
...
int there_is_an_object = 1400;

//----------------------------//

//TOP LIGHTS
#define led1 D5 //LED 1 - Top Left conrner
#define led2 D3 //LED 2 - Top Center
#define led3 D4 //LED 3 - Top Right Corner

//BOTTOM LIGHTS
#define led4 D2 //LED 4 - Bottom Center
#define led5 RX //LED 5 - Bottom Center
#define led6 TX //LED 6 - Bottom Right Corner


//----------------------------//

uint8_t leds[6] = {D5, D3, D4, D2, RX, TX};
...
unsigned long interval = 1450;

unsigned int fadeUpTime = 1100;

unsigned int fadeDownTime = 1100;
...
void setup()
{
    ...
    SoftPWMSet(led5, 0);
    SoftPWMSet(led6, 0);
    Particle.subscribe("sensor_emilia_1", string_state);
...
void string_state(const char *name, const char *data) {
    ...
    unsigned int sensor1, sensor2, sensor3, sensor4, sensor5, sensor6;
...
void led_state() {
    ...
    if (previous_state != led_states) {
        Particle.publish("sensor_emilia_2", led_states);
...
int which_ssid (String whatever) {
    String wifi_ssid = WiFi.SSID();
    Particle.publish("status_emilia_2", "Connected to:"+wifi_ssid);
    return 1;
}
...
int connectTo(String credentials) {
    ...
    Particle.publish("status_emilia_2", info);
...
int getAllSSID(String data) {
    ...
    for (int i = 0; i < found; i++) {
        String t = (ap[i].ssid);
        Particle.publish("status_emilia_2", "SSID:"+t);
    }
    return 1;
}
